...
| Phone notifications while the app is closed | **Tested on the emulator** with LAN mode (`LAN_IP`, self-hosted ntfy, no Google): a web message reached the Android app in the background and force-stopped within seconds, and Synapse's push to ntfy now gets 200 instead of `403: IP address blocked`. One app step (the push-gateway lookup, which the app refuses over plain http to an IP) was stood in for; no real phone yet ([#3](https://github.com/Orenda-Project/rumi-messenger/issues/3), [PUSH.md](docs/PUSH.md#lan-mode-school-server-on-the-school-wi-fi-no-domain)) |
