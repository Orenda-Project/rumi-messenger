...
| **The person who runs my school's IT, with no server of my own.** I want it online over HTTPS in 20 minutes. | [Rumi Messenger on Railway](docs/RAILWAY.md) |
...
`setup.sh` starts the server, the web app, the call relay and the calls server (LiveKit) on `127.0.0.1`. It creates an admin
...
| Adding a second device | **Works** with a backup code. Since v0.1.3 the Android app uses plain words ("Confirm it's you", "backup code") and a forgotten old phone no longer blocks sending. The web app says the same words ("Sign out", "Confirm it's you", "Approve on my other phone", "Enter my backup code" (shown only once the account has a backup code), "Start fresh", "Chat backup", "Waiting for this message"), hides Labs, the Create account link, "Forgot password?" and the email/phone fields, and offers one kind of call, all through Element Web's own config ([#14](https://github.com/Orenda-Project/rumi-messenger/issues/14)) |
...
| 1:1 voice and video calls | **Tested phone to web** on the default `setup.sh` stack: the Android app (emulator) called the web app, it rang, both joined, voice then two-way video, clean hang-up, call history on both sides ([CALLING.md](docs/CALLING.md)). With the app in the background, a web call now rings the phone through push (heads-up Answer/Decline, same emulator test as push). Phone calls need `http://localhost` or HTTPS: the in-app call screen blocks plain http to a LAN IP. Real phones and calls across two networks not tested yet ([#1](https://github.com/Orenda-Project/rumi-messenger/issues/1), [#6](https://github.com/Orenda-Project/rumi-messenger/issues/6)) |
...
| Phone notifications while the app is closed | **Tested on the emulator with the v0.1.4 release APK, nothing hand-set** in LAN mode (`LAN_IP`, self-hosted ntfy, no Google): fresh install, ntfy pointed at `http://<LAN_IP>:2586`, sign in; the app registered its own pusher to `http://<LAN_IP>:2586/_matrix/push/v1/notify`, and a web message reached it in the background and force-stopped, and a web call rang it (Synapse push to ntfy: 200). No real phone yet ([#3](https://github.com/Orenda-Project/rumi-messenger/issues/3), [PUSH.md](docs/PUSH.md#lan-mode-school-server-on-the-school-wi-fi-no-domain)) |
...
Calls from the phone app, and group calls and screen sharing, use [LiveKit](https://github.com/livekit/livekit) (Apache-2.0) with
Element's [lk-jwt-service](https://github.com/element-hq/lk-jwt-service) and
[Element Call](https://github.com/element-hq/element-call), started by `setup.sh` by default
(`CALLS=off` leaves them out; [CALLING.md](docs/CALLING.md)).
