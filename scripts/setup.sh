...
dc run --rm \
  ...
  --entrypoint python3 synapse - < "${DEPLOY_DIR}/synapse/patch_homeserver.py"
# ^ deploy/synapse/patch_homeserver.py: the ONE copy of this patch, also run at every boot by the
#   Railway image (deploy/railway/synapse/entrypoint.sh). Its comments explain every key.
